...
##     return(mean((X[,!is_na, drop=FALSE] %*% beta[!is_na] - Y)^2))
##   }
##   predmat <- sapply(1:num, function(x) mse(sample(1:n, floor(n/2), replace=TRUE)))
##   return(predmat)
## }
...
##       n <- length(Ytmp)
##       predmat[,i] <- sapply(1:num, function(x)
##         mse_env(sample(1:n, floor(n/2), replace=TRUE), Xtmp, Ytmp))
##     }
##     return(apply(predmat, 1, min))
##   }
## }

## bootstrap_mse <- function(Y, X, num){
##   n <- length(Y)
##   mse <- function(ind){
##     beta <- matrix(coefficients(lm.fit(X[ind,,drop=FALSE], Y[ind])), ncol=1)
##     is_na <- is.na(beta)
...
## bootstrap_mse_A <- function(Y, X, A, num){
##   Alist <- lapply(unique(A), function(a) which(A == a))
##   predmat <- matrix(NA, num, length(Alist))
##   mse <- function(ind, Xin, Yin){
##     beta <- matrix(coefficients(lm.fit(Xin[ind,,drop=FALSE], Yin[ind])), ncol=1)
##     is_na <- is.na(beta)
##     return(mean((Xin[,!is_na, drop=FALSE] %*% beta[!is_na] - Yin)^2))
##   }
...
##     n <- length(Ytmp)
##     predmat[,i] <- sapply(1:num, function(x)
##       mse(sample(1:n, floor(n/2), replace=TRUE), Xtmp, Ytmp))
##   }
##   return(apply(predmat, 2, min))
## }
